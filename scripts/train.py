...
from src.models import LightGBMModel, CrossValidationTrainer, save_model_with_metadata
from src.validation import CVLogger, check_data_integrity, validate_target_distribution
from src.util.time_tracker import WorkflowTimeTracker, WorkflowTimer
from src.util.notifications import notify_start, notify_complete, notify_error
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('outputs/logs/training.log', mode='a')
    ]
)
...
def save_results(cv_results: Dict[str, Any], feature_names: list[str]) -> None:
    ...
    log_entry = cv_logger.create_log_entry(
        ...
        prediction_distribution=cv_results['prediction_distribution'],
        feature_importance=cv_results.get('feature_importance', {}).to_dict('records') if cv_results.get('feature_importance') is not None else None
    )
...
def send_completion_notification(cv_results: Dict[str, Any]) -> None:
    ...
    try:
        ...
        notify_complete(
            ...
            metrics=metrics,
            duration=cv_results['training_time']
        )
